...
type Feature = SliderFeature | RadioFeature;

const featureDescriptions: Record<FormDataKeys, string> = {
...
export default function Predict() {
  ...
  const handleSliderChange = (name: FormDataKeys) => (_: Event, newValue: number | number[]) => {
    setFormData({ ...formData, [name]: newValue as number });
  };

  const handleRadioChange = (name: FormDataKeys) => (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.replace(
        `/result?stress_level=${prediction}&probability=${encodeURIComponent(JSON.stringify(probability))}`
      );
    } catch (err) {
  ...
  const renderInput = (feature: Feature) => {
    const { label, name } = feature;

    return (
      <Card elevation={0} sx={{ borderRadius: "8px", mb: 2 }}>
        <CardContent>
          <Box sx={{ display: "flex", alignItems: "center", mb: 1 }}>
            <Typography sx={{ fontWeight: 500, color: "#592E83", flex: 1 }}>{label}</Typography>
            <Tooltip title={featureDescriptions[name]} placement="top">
              <InfoOutlinedIcon fontSize="small" sx={{ color: "#9984D4", cursor: "pointer" }} />
            </Tooltip>
          </Box>
          {feature.type === "slider" ? (
            <>
              ...
              <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                <Typography variant="caption">{feature.minLabel}</Typography>
                <Typography variant="caption">{feature.maxLabel}</Typography>
              </Box>
            </>
          ) : (
            <FormControl>
            ...
            <Box sx={{ display: "flex", justifyContent: "center", mt: 4 }}>
              <Button
                ...
                sx={{
                  ...
                  fontSize: "1.1rem",
                  "&:hover": {
                    background: "linear-gradient(90deg, #CAA8F5 0%, #9984D4 100%)",
